...
async function seedUsers() {
  ...
  // Hash passwords for test users (matching E2E test fixtures)
  const testAdminPassword = await bcrypt.hash('Admin123!', 10);
  const testUserPassword = await bcrypt.hash('User123!', 10);
  const testViewerPassword = await bcrypt.hash('Viewer123!', 10);

  // Hash passwords for dev users
  const devAdminPassword = await bcrypt.hash('admin123', 10);
  const devUserPassword = await bcrypt.hash('dev123', 10);
  const devTesterPassword = await bcrypt.hash('test123', 10);
  const devInactivePassword = await bcrypt.hash('inactive123', 10);
...
async function main() {
  ...
  try {
    ...
    console.log('  - 7 Users (3 test users + 4 dev users)');
    console.log('    • Test: admin@test.com, user@test.com, viewer@test.com');
    console.log('    • Dev: admin@claudecode.dev, developer@claudecode.dev, tester@claudecode.dev, inactive@claudecode.dev');
